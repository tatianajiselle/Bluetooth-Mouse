...
#define THRESHOLD                 59
#define MIN_BUTTON_CLICK_LENGTH   5 // in milliseconds
...
void loop() {
  ...
  /* Check if delta values are outliers. If so, reset them to zero. */
  if(! goodDeltaValue(Sensor.dX, &prev_dX)){ Sensor.dX = 0; }
  if(! goodDeltaValue(Sensor.dY, &prev_dY)){ Sensor.dY = 0; }
	...
	if (scrollCounter != 0) {
	  ble.sendCommand(cmd += "," + String(scrollCounter));
    scrollCounter = 0;
	}
	/* ... or, just send original command (move mouse cursor only) */
	else { ble.sendCommand(cmd); }
...
bool goodDeltaValue(int curr, int *prev){
  int diff = toSignedInt(curr) - toSignedInt(*prev);  
  *prev = curr;
  if(abs(diff) > THRESHOLD){ return false; }
  return true;
}

void buttonState(int buttonPin, String button, bool *buttonIsPressed) {
  if (digitalRead(buttonPin) == HIGH && !(*buttonIsPressed)) {
    if(millis() - buttonTimer > MIN_BUTTON_CLICK_LENGTH){
      buttonTimer = millis();
...
void resetSettings() {
	...
	ble.sendCommandCheckOK(F("AT+GAPDevName=MaviMouse"));
